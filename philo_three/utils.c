...
/*   Updated: 2020/11/01 09:17:17 by fhenrion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			exit_error(t_error error, t_data *state)
{
	...
	else if (error == SEMAPHORE_ERROR)
		write(STDERR, "Semaphore creation error\n", 25);
	else if (error == THREAD_ERROR)
		write(STDERR, "Thread creation error\n", 22);
	else if (error == THREAD_ERROR)
		write(STDERR, "Child creation error\n", 21);
	else
		write(STDERR, "Unimplemented error ?\n", 22);
	if (state)
		unlink_semaphores(state->parameters->philo_nb);
	return ((int)error);
}
